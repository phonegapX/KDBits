// OpPropertySheet.cpp : 实现文件
//

#include "stdafx.h"
#include "..\KDBits.h"
#include "OpPropertySheet.h"

// COpPropertySheet
static BOOL bMoved[4] = {FALSE,FALSE,FALSE,FALSE};

IMPLEMENT_DYNAMIC(COpPropertySheet, CPropertySheet)

COpPropertySheet::COpPropertySheet(UINT nIDCaption, CWnd* pParentWnd, UINT iSelectPage)
...
COpPropertySheet::COpPropertySheet(LPCTSTR pszCaption, CWnd* pParentWnd, UINT iSelectPage)
...
COpPropertySheet::~COpPropertySheet()
{
	delete [] m_szCaption;
}


BEGIN_MESSAGE_MAP(COpPropertySheet, CPropertySheet)
	ON_WM_PAINT()
...
// COpPropertySheet 消息处理程序

BOOL COpPropertySheet::OnInitDialog()
{
...
void COpPropertySheet::DrawCaption(CDC * pDC, const COLORREF clrCaption)
{
	CDC dcBuf;
...
void COpPropertySheet::OnNMClick(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void COpPropertySheet::InitList(void)
{
	LVITEM lvi;
...
void COpPropertySheet::OnNMCustomDraw(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void COpPropertySheet::DrawGradientLine(CDC* pDC, COLORREF clrLine, POINT ptStart, POINT ptEnd)
{
...
int COpPropertySheet::AddIcon(HICON icon)
{
	return m_imgList.Add(icon);
}

void COpPropertySheet::SetCaptionColor(const COLORREF clrCaption)
{
	m_clrCaption = clrCaption;
}

void COpPropertySheet::SetSepratorColor(const COLORREF clrSeprator)
{
	m_clrSeprator = clrSeprator;
}

void COpPropertySheet::SetListFont(CFont * pFont)
{
	m_ftList.Attach(pFont->GetSafeHandle());
}

void COpPropertySheet::SetSelectedColor(const COLORREF clrSelected)
{
